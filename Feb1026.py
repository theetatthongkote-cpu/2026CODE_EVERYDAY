...
class Character:
    ...
    def heal(self, amount=None):
        if self.heal_cooldown > 0:
            ...
            return
        if self.health <= 0:
            print(f"{self.name} cannot heal (defeated).")
            return
        if amount is None:
    ...
    def player_turn(player, enemy):
        ...
        print(
            f"Cooldowns → Heal:{player.heal_cooldown} | FullBlock:{player.full_block_cooldown} | HalfBlock:{player.half_block_cooldown}")
    ...
    def show_inventory(self):
        ...
        for i, (item, qty) in enumerate(self.inventory.items(), 1):
    ...
    def use_item(self, item, enemy=None):
        ...
        if item.isdigit() and 1 <= int(item) <= len(items):
            selected_item = items[int(item) - 1]
            player.use_item(selected_item, enemy)
        else:
